/* 
 * Purpose:
 *   Test case to demonstrate that statement execution in Oracle
 *   may trigger a dml retry followed by a write-consistency restart.
 *
 *   The test case simulates the following scenario:
 *
 *   Session 1: execute an update on a partitioned table.
 *
 *   Session 2: rebuild an index while the update in session 1 is
 *              in-flight to trigger an internal ORA-14403 error
 *              and then modify a row that hasn't been processed
 *              by the update in session 1 to trigger a write-
 *              consistency restart.
 *
 * Author:
 *   Christoph Lutz
...
create or replace package pkg_tc_restart_09 as
    G_DEBUG      boolean               :=  FALSE;
    G_STARTS     pls_integer           :=  0;
    PIPE_SES1    constant varchar2(32) := 'PIPE_SES1';
    PIPE_SES2    constant varchar2(32) := 'PIPE_SES';
...
end pkg_tc_restart_09;
/

create or replace package body pkg_tc_restart_09 as
    ...
    begin
        loop
            ...
            if l_msg = CMD_UPDATE then
                ...
                update tc_restart set n = 7 where n = 6 returning n into l_n;
                commit;
            ...
            /* 
             * Rebuild index partition P09_12 to avoid getting 
             * stuck on 'enq: TX - row lock contention'.
             */
            if l_msg = CMD_REBUILD then
                debug(to_char(systimestamp, 'hh24:mi:ss.ff9') ||': ses2: rebuilding index ...');
                execute immediate 'alter index i_n_tc_restart rebuild partition P09_12 online';
                send(PIPE_SES1, CMD_CONTINUE);
