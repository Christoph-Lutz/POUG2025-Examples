...
create or replace package body pkg_tc_restart_02 as
    ...
    begin
         ...
         */
        if p_n = 1 and G_PASS <= G_PASSES then
        ...
        debug(to_char(systimestamp, 'hh24:mi:ss.ff9') ||': ses1: test_func: G_PASS=' ||G_PASS ||' ret: p_n='||p_n);
        return p_n;
    end test_func;

    procedure start_ses1 as
    begin
        update /*+ monitor */ tc_restart t set t.n = n+1 where test_func(t.n) > 0;
    end start_ses1;
