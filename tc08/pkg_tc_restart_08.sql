 ...
 *   may trigger a write-consistency restart followed by a dml retry.
 *
 *   The test case simulates the following scenario:
 *
 *   Session 1: execute an update on a partitioned table.
 *
 *   Session 2: modify a row in the test table not yet processed by
 *              the update in session 1 and then rebuild an index
 *              after the update in session 1 has restarted.
 *              The index rebuild will trigger a dml retry.
 *
 * Author:
 *   Christoph Lutz
 *
...
create or replace package pkg_tc_restart_08 as
...
end pkg_tc_restart_08;
/

create or replace package body pkg_tc_restart_08 as
    ...
    begin
        loop
            ...
            if l_msg = CMD_UPDATE then
                ...
                update tc_restart set n = 3 where n = 2 returning n into l_n;
                commit;
