/* 
 * Purpose:
 *   Test case to demonstrate Oracle dml retry behavior by simulating
 *   the following scenario:
 *
 *   Session 1: execute an update on a partitioned table.
 *
 *   Session 2: rebuild an index on the partitioned table while
 *              the update in session 1 is still in progress.
 *
 *   The index rebuild in session 2 will invalidate the update cursor
 *   that session 1 is executing, which will trigger a dml retry.
 *
 * Author:
 *   Christoph Lutz
 *
...
create or replace package pkg_tc_restart_07 as
    G_DEBUG       boolean               :=  FALSE;
    G_RETRIES     pls_integer           :=  0;
    G_RETRIES_MAX pls_integer           :=  42;
...
end pkg_tc_restart_07;
/

create or replace package body pkg_tc_restart_07 as
    ...
    begin
         ...
         */
        if p_n = 1 and G_RETRIES < G_RETRIES_MAX then
    ...
    begin
        loop
            ...
            if l_msg = CMD_REBUILD then
                ...
                execute immediate 'alter index i_n_tc_restart rebuild partition P5 online';
